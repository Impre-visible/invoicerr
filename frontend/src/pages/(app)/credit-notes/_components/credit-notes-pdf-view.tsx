...
export function ReceiptPdfModal({ receipt, onOpenChange }: ReceiptPdfModalProps) {
  ...
  const { data } = useGetRaw<Response>(`/api/credit-notes/${receipt?.id}/pdf`)
  ...
  return (
    ...
    >
      <DialogContent className="!max-w-none w-fit min-w-[90vw] md:min-w-128 h-[90dvh] overflow-hidden flex flex-col">
        <DialogHeader>
          <DialogTitle>{t("credit-notes.pdf.title", { number: receipt.rawNumber || receipt.number })}</DialogTitle>
        </DialogHeader>
          ...
          {pdfData ? (
            <div className="flex justify-center h-full overflow-auto">
              <iframe
                ...
                title={t("credit-notes.pdf.title", { number: receipt.rawNumber || receipt.number })}
              />
            </div>
          ) : (
